...
function HelpCenterPage() {
  ...
  const fetchHelpRequests = async (token, filters = {}) => {
    try {
      ...
      try {
        ...
        setError(`Failed to fetch help requests: ${apiError.response?.status} ${apiError.response?.statusText || apiError.message}`);
        
        // For testing: Add some dummy data if the API fails
  ...
  useEffect(() => {
    ...
    fetchUserData();
  }, [fetchHelpRequests, searchQuery, searchTags, selectedCourse]);
