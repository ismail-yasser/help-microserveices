...
function HelpCenterPage() {
  ...
  const fetchHelpRequests = useCallback(async (token, filters = {}) => {
    try {
      let data;
      // Logic uses searchQuery, searchTags, selectedCourse from component state
      if (searchQuery || (searchTags && searchTags.length > 0) || selectedCourse) {
      ...
      console.error('API Error in fetchHelpRequests:', apiError);
      throw apiError; 
    }
  }, [searchQuery, searchTags, selectedCourse]);
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        console.error('Error in fetchUserData (useEffect):' + err);
        let errorMessage = 'Failed to load help requests.';
        if (err.response) {
          errorMessage = `Failed to load help requests: ${err.response.status} ${err.response.statusText || ''}`;
        } else if (err.message) {
          errorMessage = `Failed to load help requests: ${err.message}`;
        }
        ...
        if (process.env.NODE_ENV !== 'production') {
          setHelpRequests([
            {
              _id: 'dummy1',
              subject: 'Help with React',
              description: 'I need help with React component lifecycle',
            ...
            {
              _id: 'dummy2',
              subject: 'MongoDB Query Help',
              description: 'How do I create a complex aggregation pipeline?',
...
function calculatePoints(urgency, estimatedTimeInMinutes = 30) {
  ...
  const timeMultiplier = Math.ceil(estimatedTimeInMinutes / 15);
