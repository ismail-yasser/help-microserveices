...
for /f "tokens=*" %%i in ('kubectl get secret docker-registry-creds -n production -o jsonpath="{.data.\.dockerconfigjson}" ^| base64 --decode ^| jq -r ".auths[\"https://index.docker.io/v1/\"].username"') do (
    set DOCKER_USERNAME=%%i
)
