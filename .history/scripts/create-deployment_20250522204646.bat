...
if "%ALL_SERVICES%"=="true" (
    ...
    if exist "%K8S_DIR%\frontend-deployment.yaml" (
        ...
        if !errorlevel! neq 0 (
            echo WARNING: Failed to apply frontend deployment.
        ) else (
            echo Successfully applied frontend deployment.
            kubectl rollout status deployment/frontend
        )
    )
    
    echo Applying help-service deployment...
    if exist "%K8S_DIR%\help-service-deployment.yaml" (
        kubectl apply -f "%K8S_DIR%\help-service-deployment.yaml"
        if !errorlevel! neq 0 (
            echo WARNING: Failed to apply help-service deployment.
        ) else (
            echo Successfully applied help-service deployment.
            kubectl rollout status deployment/help-service
        )
    )
    
    echo Applying user-service deployment...
    if exist "%K8S_DIR%\user-service-deployment.yaml" (
        kubectl apply -f "%K8S_DIR%\user-service-deployment.yaml"
        if !errorlevel! neq 0 (
            echo WARNING: Failed to apply user-service deployment.
        ) else (
            echo Successfully applied user-service deployment.
