...
kubectl get deployment user-service >nul 2>&1
if %errorlevel% equ 0 (
    echo   ✅ User Service deployment is running
    kubectl get deployment user-service | findstr "2/2"
) else (
    echo   ❌ User Service deployment not found
)

kubectl get deployment help-service >nul 2>&1
if %errorlevel% equ 0 (
    echo   ✅ Help Service deployment is running
    kubectl get deployment help-service | findstr "2/2"
) else (
    echo   ❌ Help Service deployment not found
)

kubectl get deployment frontend >nul 2>&1
if %errorlevel% equ 0 (
    echo   ✅ Frontend deployment is running
    kubectl get deployment frontend | findstr "2/2"
) else (
