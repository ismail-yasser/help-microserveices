...
for /l %%i in (1,1,17) do (
    ...
    if %%i lss 10 (
        call "scripts\task-validation\0%%i-check-*.bat" >nul 2>&1
    ) else (
        call "scripts\task-validation\%%i-check-*.bat" >nul 2>&1
    )
...
if %task_choice% geq 1 if %task_choice% leq 17 (
    ...
    if %task_choice% lss 10 (
        call "scripts\task-validation\0%task_choice%-check-*.bat"
    ) else (
        call "scripts\task-validation\%task_choice%-check-*.bat"
    )
...
for /l %%i in (1,1,17) do (
    ...
    if %%i lss 10 (
        call "scripts\task-validation\0%%i-check-*.bat"
    ) else (
        call "scripts\task-validation\%%i-check-*.bat"
    )
