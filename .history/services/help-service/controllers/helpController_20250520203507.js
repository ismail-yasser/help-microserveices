...
const awardPoints = async (userId, points, action) => {
  try {
    // Using the correct port for gamification service
    await axios.post(`http://localhost:3009/api/gamification/${userId}/points`, {
      points,
      action
    });
    return true;
...
exports.getHelpById = async (req, res) => {
  try {
    ...
    console.error('Error getting help item:', error);
    res.status(500).send('Error getting help item');
  }
};
...
exports.deleteHelp = async (req, res) => {
  try {
    ...
    res.status(200).send('Help item deleted successfully');
  } catch (error) {
