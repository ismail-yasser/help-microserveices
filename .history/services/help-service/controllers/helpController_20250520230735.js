  ...
  try {
    ...
    if (req.files && req.files.length > 0) {
      fileAttachments = req.files.map(file => ({
        ...
        path: file.path, // Store the path relative to the server
    ...
    const helpRequest = new Help({
      ...
      fileAttachments, // Add file attachments to the model
      type: 'request',
...
exports.getHelpById = async (req, res) => {
  try {
    const helpItem = await Help.findById(req.params.id).populate('userId', 'name email').populate('offers.userId', 'name email');
    if (!helpItem) {
