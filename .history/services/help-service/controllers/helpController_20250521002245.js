...
exports.getAllHelp = async (req, res) => {
  try {
    ...
    // Fetch user profiles to get names for each userId
    try {
      const userResponse = await axios.get('http://localhost:3000/api/users/profiles');
      const users = userResponse.data || [];
      
      // Add the requester name to each help item
      const enrichedHelpItems = helpItems.map(item => {
        const user = users.find(u => u._id && item.userId && u._id.toString() === item.userId.toString());
        return {
          ...item._doc,
      ...
      // Fall back to returning items without user names
