...
const awardPoints = async (userId, points, action) => {
  try {
    // TODO: Uncomment when gamification-service is active
    // await axios.post(`http://localhost:3003/api/gamification/${userId}/points`, {
    //   points,
    //   action
    // });
    console.log(`Skipping awardPoints for ${userId} - gamification-service not active.`);
    return true;
  } catch (error) {
    console.error('Error awarding points (gamification-service not active or error):', error);
    return false;
  }
};
...
exports.createHelpRequest = async (req, res) => {
  try {
    ...
    // TODO: Uncomment when notification-service is active
    ...
    //     });
    //   } catch (err) {
    //     console.error('Error sending notification (notification-service not active or error):', err);
    //   }
    // });
    console.log('Skipping notifications for suggested helpers - notification-service not active.');
...
exports.createHelpOffer = async (req, res) => {
  try {
    ...
    //   });
    // } catch (err) {
    //   console.error('Error sending new offer notification (notification-service not active or error):', err);
    // }
    console.log('Skipping new offer notification - notification-service not active.');
...
exports.acceptHelpOffer = async (req, res) => {
  try {
    ...
    //   });
    // } catch (err) {
    //   console.error('Error sending offer accepted notification (notification-service not active or error):', err);
    // }
    console.log('Skipping offer accepted notification - notification-service not active.');
    
    // Award points to the helper who got accepted (initial points for acceptance)
    // More points can be awarded upon resolution
    console.log(`Skipping awardPoints for ${offer.userId} (offer_accepted) - gamification-service not active.`);
...
exports.getHelpById = async (req, res) => {
  try {
    // Check if ID is undefined, null, or not a valid ObjectId
    ...
    res.status(500).json({ message: 'Error fetching help item' });
  }
};
...
exports.addResponse = async (req, res) => {
  try {
    ...
    //   console.error('Error sending notification (notification-service not active or error):', err);
    // }
    console.log('Skipping help response notification - notification-service not active.');
  ...
  try {
    ...
    if (status === 'resolved' && helpItem.status !== 'resolved') {
      for (const helperId of helpItem.assignedHelpers) {
        ...
        //   console.error('Error sending notification (notification-service not active or error):', err);
        // }
        console.log(`Skipping points awarded notification for ${helperId} - notification-service not active.`);
      }
    }
...
exports.assignHelper = async (req, res) => {
  try {
    ...
    //   console.error('Error sending notification (notification-service not active or error):', err);
    // }
    console.log(`Skipping assign helper notification for ${helperId} - notification-service not active.`);
  ...
  try {
    ...
    for (const participantId of participantIds) {
      if (participantId !== req.user.id) {
        ...
        //   console.error('Error sending notification (notification-service not active or error):', err);
        // }
        console.log(`Skipping meeting scheduled notification for ${participantId} - notification-service not active.`);
      }
    }
...
exports.rateResponse = async (req, res) => {
  try {
    ...
    if (rating >= 4) {
      ...
      //   console.error('Error sending notification (notification-service not active or error):', err);
      // }
      console.log(`Skipping bonus points notification for ${response.userId} - notification-service not active.`);
    }
