...
function CreateHelpRequestPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTagList([]);
      setUrgency('medium');
      setEstimatedTimeInMinutes(30);
      setRequiredSkillInput('');
      setRequiredSkills([]);
      setAttachments([]);
      
      // Navigate to the new request page after a brief delay
      // Ensure we access the correct property from the response
      const helpRequestId = response.data.helpRequest._id;
      console.log('Created help request with ID:', helpRequestId);
      
      setTimeout(() => {
        navigate(`/request/${helpRequestId}`);
      }, 1500);
    } catch (err) {
