...
powershell -Command "(Get-Content k8s/frontend-deployment.yaml) -replace 'frontend:local', '%DOCKER_USERNAME%/frontend:latest' | Set-Content k8s/frontend-deployment-prod.yaml"
powershell -Command "(Get-Content k8s/user-service-deployment.yaml) -replace 'user-service:local', '%DOCKER_USERNAME%/user-service:latest' | Set-Content k8s/user-service-deployment-prod.yaml"  
powershell -Command "(Get-Content k8s/help-service-deployment.yaml) -replace 'help-service:local', '%DOCKER_USERNAME%/help-service:latest' | Set-Content k8s/help-service-deployment-prod.yaml"

kubectl apply -f k8s/frontend-deployment-prod.yaml -n production
kubectl apply -f k8s/user-service-deployment-prod.yaml -n production
kubectl apply -f k8s/help-service-deployment-prod.yaml -n production
